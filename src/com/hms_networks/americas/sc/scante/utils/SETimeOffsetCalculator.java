package com.hms_networks.americas.sc.scante.utils;
 ...
 * @version 1.0
 */
public class SETimeOffsetCalculator {
  ...
  private static void writeLocalTimeHTMLFile() {
    File file = new File(SEConnectorConsts.TIME_OFFSET_HTML_FILE_NAME);
  ...
  private static void generateLocalTimeFile() {
    ...
    try {
      ScheduledActionManager.GetHttp(
          ...
          SEConnectorConsts.TIME_OFFSET_RESPONSE_FILE_NAME,
          SEConnectorConsts.TIME_OFFSET_HTML_FILE_NAME);
    } catch (EWException e) {
      Logger.LOG_SERIOUS(
  ...
  private static long parseOffsetFromLocalTime() {
    ...
    try {
      reader = new BufferedReader(new FileReader(SEConnectorConsts.TIME_OFFSET_RESPONSE_FILE_NAME));
    ...
    SimpleDateFormat sdf = new SimpleDateFormat(SEConnectorConsts.TIME_OFFSET_DATE_FORMAT);
